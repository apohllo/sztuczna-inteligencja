...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# // skomentuj tutaj
# W powyższym modelu bardziej liczy się precision niż recall, gdyż bardziej zależy nam na poprawnym zidentyfikowaniu zainteresowanych/niezainteresowanych klientów, by uniknąć adresowania reklamy do tych niezainteresowanych (zidentyfikowani zainteresowani rzeczywiście są zainteresowani). Recall natomiast skupiałby się na sprawdzaniu, jaki procent zainteresowanych model zidentyfikował. Na tym nie zależy nam aż tak - jeśli jakiś klient zainteresowany nie zostanie zidentyfikowany(niski )recall), nic się nie stanie. Natomiast jeśli klient niezainteresowany dostanie ofertę (niski precision), spowodujemy niechciany w marketingu efekt, a klient może poczuć się sfrustrowany niechcianą reklamą i zmienić swoją opinię o banku na złą. 
#
#
#

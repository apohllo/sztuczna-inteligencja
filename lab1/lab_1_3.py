...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# // skomentuj tutaj
# Błąd testowy jest dużo większy od treningowego - następuje overfitting. Oznacza to, że model słabo radzi sobie z dopasowywaniem się do nowych danych (generalizacją). Powyższe błędy są też stosunkowo duże - na wykresie zatytułowanym "House area vs price, outliers removed" możemy zobaczyć, że ceny domów wahają się od około 100tys. dolarów do około 600tys. dolarów, natomiast błąd uzyskany z danych testowych osiągnął ponad 21tys. - jest to dość spora wartość względna. Jeśli przyjmiemy za średnią wartość domu 350 000$, to wtedy błąd względny wyniesie w przybliżeniu 6%, natomiast jeśli jeszcze zwrócimy uwagę, że większość domów ma cenę bliższą 300 000\$, to błąd ten jeszcze wzrośnie.
#
#
...
reg_ridge_cv=RidgeCV(alphas=np.linspace(0.1,100,1000)).fit(X_train, y_train)
reg_lasso_cv=LassoCV(cv=1000,random_state=0).fit(X_train, y_train)

print("Ridge:")
ridge_train_rmse,ridge_test_rmse=assess_regression_model(reg_ridge_cv, X_train, X_test, y_train, y_test)
print("alpha: ",round(reg_ridge_cv.alpha_,4))

print()

print("Lasso:")
lasso_train_rmse,lasso_test_rmse=assess_regression_model(reg_lasso_cv, X_train, X_test, y_train, y_test)
print("alpha: ",round(reg_lasso_cv.alpha_,4))
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# // skomentuj tutaj   
# Dla obu rodzajów walidacji skrośnej nadal otrzymujemy mniejszy błąd dla danych treningowych niż testowych, jednak różnica pomiędzy wartościami błędów jest bardzo mała zarówno w przypadku RidgeCV, jak i LassoCV. Dzięki wykorzystaniu CV uzyskaliśmy bardziej miarodajną walidację obu modeli, gdyż przetestowaliśmy je na różnych zbiorach danych. Sprawiło to, że otrzymane błędy okazały się nieco wyższe dla regularyzacji L2, jednak znacząco zmalały (i pozwoliły zlikwidować underfitting) dla regularyzacji L1. Możemy uznać, że LassoCV jest w tym przypadku lepszym modelem, ponieważ RMSE danych testowych jest mniejsze niż w przypadku RidgeCV, a także różnica pomiędzy train_rmse i test_rmse jest mniejsza niż dla regularyzacji L2.
#
#
...
print("Result is correct") #Added - was not present here
...
# plot missing values
msno.bar(df)
df.shape #Z porównania wyników działania obu funkcji widzimy, że wszystkie cechy są kompletne
...
l0_regression=LogisticRegression(class_weight="balanced",penalty=None, random_state=0)
l0_regression.fit(X_train,y_train)
y0_pred=l0_regression.predict(X_test)

l0_precision=precision_score(y_test,y0_pred)
l0_recall=recall_score(y_test,y0_pred)
l0_f1=f1_score(y_test,y0_pred)

l2_regression=LogisticRegressionCV(class_weight="balanced",cv=5,Cs=100, random_state=0,n_jobs=-1,scoring="f1")
l2_regression.fit(X_train,y_train)
y2_pred=l2_regression.predict(X_test)

l2_precision=precision_score(y_test,y2_pred)
l2_recall=recall_score(y_test,y2_pred)
l2_f1=f1_score(y_test,y2_pred)

l1_regression= LogisticRegressionCV(
    Cs=100, cv=5, scoring='f1', solver='saga', penalty='l1', class_weight='balanced', random_state=0, n_jobs=-1
)
l1_regression.fit(X_train,y_train)
y1_pred=l1_regression.predict(X_test)

l1_precision=precision_score(y_test,y1_pred)
l1_recall=recall_score(y_test,y1_pred)
l1_f1=f1_score(y_test,y1_pred)

print("\t\tL0\t\tL1\t\tL2")
print(f"precision\t{l0_precision:.4f}\t\t{l1_precision:.4f}\t\t{l2_precision:.4f}")
print(f"recall   \t{l0_recall:.4f}\t\t{l1_recall:.4f}\t\t{l2_recall:.4f}")
print(f"f1       \t{l0_f1:.4f}\t\t{l1_f1:.4f}\t\t{l2_f1:.4f}")
...
print("Solution is correct!") #Added

# %% editable=true slideshow={"slide_type": ""} tags=["ex"]
y_pred_train=l0_regression.predict(X_train)
y_pred_test=l0_regression.predict(X_test)
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# // skomentuj tutaj   
# W powyższym modelu bardziej liczy się precision niż recall, gdyż bardziej zależy nam na poprawnym zidentyfikowaniu zainteresowanych klientów, by uniknąć adresowania reklamy do tych niezainteresowanych (chcemy uzyskać sytuację, gdy zidentyfikowani zainteresowani rzeczywiście są zainteresowani). Recall natomiast skupiałby się na sprawdzaniu, jaki procent zainteresowanych model zidentyfikował. Na tym nie zależy nam aż tak - jeśli jakiś klient zainteresowany nie zostanie zidentyfikowany(niski recall), nic się nie stanie. Natomiast jeśli klient niezainteresowany dostanie ofertę (niski precision), spowodujemy niechciany w marketingu efekt, a klient może poczuć się sfrustrowany niechcianą reklamą i zmienić swoją opinię o banku na złą. Chcemy więc uzyskać wysokie precision.
#
#
#
# W tym przypadku tworzenie modeli z regularyzacją nie ma większego sensu - jak widzimy powyżej, metryki niewiele różnią się od siebie dla poszczególnych modeli. 
#
...
l0_regression=LogisticRegression(class_weight="balanced",penalty=None, random_state=0, max_iter=500)
l0_regression.fit(X_train, y_train)

y_train_pred = l0_regression.predict(X_train)
y_test_pred = l0_regression.predict(X_test)
...
print("L0 - F1 Score:")
print(f"Train: {100*f1_train:.4f}%")
print(f"Test: {100*f1_test:.4f}%")

l2_regression=LogisticRegressionCV(class_weight="balanced",cv=5,Cs=100, random_state=0,n_jobs=-1,scoring="f1",max_iter=500)
l2_regression.fit(X_train,y_train)
y2_pred=l2_regression.predict(X_test)
l2_f1_test=f1_score(y_test,y2_pred)
y2_pred=l2_regression.predict(X_train)
l2_f1_train=f1_score(y_train,y2_pred)
...
print("L2 - F1 Score:")
print(f"Train: {100*l2_f1_train:.4f}%")
print(f"Test: {100*l2_f1_test:.4f}%")
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# // skomentuj tutaj
# Różnica pomiędzy f1_score dla zbioru treningowego oraz testowego jest niewielka, więc nie musimy stosować regularyzacji - nie występuje overfitting, ani underfitting. Po zastosowaniu regularyzacji l2 widzimy też, że wynik nie zmienił się w znaczący sposób - 
#
...
# Wczytanie danych
# df = pd.read_csv('AmesHousing.csv')  # Upewnij się, że dane są wczytane
# df = df.select_dtypes(include=[np.number])  # Wyciągnij tylko cechy numeryczne

# Przygotowanie danych
X = df.drop(columns='SalePrice')  # 'SalePrice' jako zmienna docelowa
y = df['SalePrice']

# Dodanie stałej dla modelu OLS
X = sm.add_constant(X)

# Krok 2: Model bez regularyzacji
model_ols = sm.OLS(y, X).fit()

# Wyświetlenie wyników
print(model_ols.summary())

# Krok 3: Model z regularyzacją L1 i L2
# Podział danych na zbiór treningowy i testowy
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.25, random_state=0)

# Model Lasso (L1)
lasso = Lasso(alpha=0.1)  # Wartość regularyzacji do dostosowania
lasso.fit(X_train, y_train)

# Model Ridge (L2)
ridge = Ridge(alpha=0.1)  # Wartość regularyzacji do dostosowania
ridge.fit(X_train, y_train)

# Krok 4: Testowanie założeń
# 1. Normalność błędów - Jarque-Bera test
jb_test = jarque_bera(model_ols.resid)
print(f"Jarque-Bera test: statystyka = {jb_test[0]:.4f}, p-value = {jb_test[1]:.4f}")

# 2. Homoskedastyczność - Breusch-Pagan test
bp_test = het_breuschpagan(model_ols.resid, model_ols.model.exog)
print(f"Breusch-Pagan test: statystyka = {bp_test[0]:.4f}, p-value = {bp_test[1]:.4f}")

# 3. Niezależność błędów - Harvey Collier test
hc_test = linear_harvey_collier(model_ols)
print(f"Harvey Collier test: statystyka = {hc_test[0]:.4f}, p-value = {hc_test[1]:.4f}")

# 4. Współliniowość - współczynnik uwarunkowania
condition_number = np.linalg.cond(X)
print(f"Współczynnik uwarunkowania: {condition_number:.4f}")

# Graficzna analiza rozkładu reszt
sns.histplot(model_ols.resid, bins=30, kde=True)
plt.title('Histogram reszt')
plt.xlabel('Reszty')
plt.ylabel('Częstość')
plt.show()

# Graficzna analiza homoskedastyczności
plt.scatter(model_ols.fittedvalues, model_ols.resid)
plt.axhline(y=0, color='r', linestyle='--')
plt.title('Reszty względem wartości przewidywanych')
